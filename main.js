...
function add_content_text(sectionID, TextWithTranslations) {
   ...
   for (let i = 0; i < TextWithTranslations.number_of_sections; i++) {
      ...
      const textNode = document.createTextNode(TextWithTranslations.sections[i].title);
      title.appendChild(textNode)   

      for (let j = 0; j < TextWithTranslations.sections[i].paragraphs.length; j++){
      ...
      const textNode = document.createTextNode(TextWithTranslations.sections[i].paragraphs[j]);
...
function add_cognates(TextWithTranslations){
   ...
   for (let i = 0; i < TextWithTranslations.cognates.length; i++){
      ...
      for (let j = 0; j < TextWithTranslations.number_of_paragraphs; j++){
         ...
         const split_sentence = TextWithTranslations.separate_sentence_into_words(sentence);
...
function show_cognate_translation(num) {
   const cognate_translation_span = document.getElementById(`cognate_translation${num}`);
   ...
   for (let i = 0; i < TextWithTranslations.challenging_words.length; i++){
      ...
      for (let j = 0; j < TextWithTranslations.number_of_paragraphs; j++){
         ...
         if (challenging_word_index !== -1) {
            ...
            split_sentence.splice(challenging_word_index,1,challenging_word_span_container)
            const joined_sentence = split_sentence.join("");
...
function show_challenging_word_translation(num) {
   const challenging_word_translation_span = document.getElementById(`challenging_word_translation${num}`);
